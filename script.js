...
async function getsongs(folder){
    ...
    for (const song of songs) {
        songUL.innerHTML=songUL.innerHTML + `<li> 
                ...
                <div class="playnow">
                  <span>Play Now</span>
                  <img src="img/play.svg" alt="Play Now" class="invert" />
                </div>
               </li>`
    }
...
async function main(){
    ...
    currentsong.pause();
    const filename = currentsong.src.split("/").slice(-1)[0]; // Extract filename
    let index = songs.findIndex(song => song === filename); // Use a callback
    if (index - 1 >= 0) {
